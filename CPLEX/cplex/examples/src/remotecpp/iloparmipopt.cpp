...
protected:
    ...
    */
   std::streamsize xsputn (const char* s, std::streamsize n) ILO_OVERRIDE {
    ...
    */
   int overflow (int c = EOF) ILO_OVERRIDE {
      if ( c == EOF )
         return EOF;
      else {
...
#   error "No transport type selected"
   ...
   for (int i = 1; i < argc; ++i) {
      ...
      //   we connect (via ssh)
      // -bin=<binary> specifies the binary to execute (default is "cpxworker")
