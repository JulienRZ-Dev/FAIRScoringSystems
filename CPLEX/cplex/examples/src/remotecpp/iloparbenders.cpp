...
#undef NCUSTOMER
   ...
   // interface.
   IloInt getNBlocks() const ILO_OVERRIDE { return nblocks; }
   IloModel getModel() const ILO_OVERRIDE { return model; }
   IloNumVarArray getVariables() const ILO_OVERRIDE { return vars; }
   IloRangeArray getRows() const ILO_OVERRIDE { return ranges; }
   IloInt getBlock(IloNumVar x) const ILO_OVERRIDE {
      BlockMap::const_iterator const it = blockMap.find(x);
      return (it == blockMap.end()) ? -1 : it->second;
   }
   RowSet const &getIntersectedRows(IloNumVar x) const ILO_OVERRIDE {
   ...
   double getObjCoef(IloNumVar x) const ILO_OVERRIDE {
      ObjMap::const_iterator const it = objMap.find(x);
      return (it == objMap.end()) ? 0.0 : it->second;
   }
   IloObjective::Sense getObjSense() const ILO_OVERRIDE { return objSense; }
};
