...
// --------------------------------------------------------------------------
 ...
 {int} forcedCoveredFlights = ...;
 int costMax = ...;
 int tabuLength = ...; // crude way to avoid finding the same
 
 {TPairingLeg} genLegs;
 TPairingCost genCost;
 ...
 execute DEBUG {
   if ( PairingGenerationData.DebugMode >= 1 ) {
     writeln("-- searching for pairings with cost < ", costMax, ", length !=", tabuLength, ", forced=", forcedCoveredFlights); 
    }
 }
...
subject to
 {
    ...
    costVar == max(j in 1..3) costVars[j];
     ...
     // at least one forced flight is covered.
     if ( card(forcedCoveredFlights) > 0)
       sum (f in forcedCoveredFlights) count(flightVars, f ) >= 1;
