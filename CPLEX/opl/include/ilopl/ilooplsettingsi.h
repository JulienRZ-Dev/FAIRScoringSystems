...
public:
	...
	void setWithJavaScript(IloBool flag);

	IloBool isDo4dsxDebug() const;
	void setDo4dsxDebug(IloBool flag);
...
public:
    ...
    std::ostream& getOut() const ILO_OVERRIDE;

    void enableAbort(IloBool enable) ILO_OVERRIDE;
    void abort() ILO_OVERRIDE;

    IloBool overrideBlock(const char* name) ILO_OVERRIDE;
    IloBool overrideMain(IloInt& status) ILO_OVERRIDE;

    void notifyNew(const IloOplModelI& opl) ILO_OVERRIDE;
    void notifyEnd(const IloOplModelI& opl) ILO_OVERRIDE;
    void notifyCall(const IloOplModelI& opl) ILO_OVERRIDE;
    void notifyReturn(const IloOplModelI& opl, IloBool status) ILO_OVERRIDE;

    void notifyNew(IloAlgorithmI& algorithm) ILO_OVERRIDE;
    void notifyEnd(IloAlgorithmI& algorithm) ILO_OVERRIDE;
    void notifyCall(IloAlgorithmI& algorithm) ILO_OVERRIDE;
    void notifyReturn(IloAlgorithmI& algorithm, IloBool status) ILO_OVERRIDE;
};
