...
public:
    ...
    virtual IloBool read() ILO_OVERRIDE {
        return IloTrue;
    }

    virtual const IloOplModelSourceI& getModelSource() const ILO_OVERRIDE;
    virtual void parse() ILO_OVERRIDE;
    virtual void check() ILO_OVERRIDE;
...
public:
    ...
    IloOplInputSourceI(IloEnvI* env, std::istream& ins, const char* name);
    ~IloOplInputSourceI();
...
protected:
  virtual std::istream& openStream() ILO_OVERRIDE;
  virtual void closeStream() ILO_OVERRIDE;
public:
    IloOplModelSourceI(IloEnvI* env, const char* filename);
    IloOplModelSourceI(IloEnvI* env, std::istream& ins, const char* name);
...
public:
    ...
    void compile(const IloOplModelSourceI& input, std::ostream& outs);

    static void MarkCompiled (std::ostream& outs);
    static IloBool IsCompiled(std::istream& ins);
};
