...
#define REDEFINE_SIMPLE_ELT_MAP_ACCESSORS_BOUND(_eltType)\
   \
  _eltType get(IloInt index) const{\
    return super::get(index);\
  }\
   \
  _eltType get(IloNum index) const{\
    return super::get(index);\
  }\
   \
  _eltType get(const char* index) const{\
    return super::get(index);\
  }\
  _eltType get(IloSymbol index) const{\
    return super::get(index);\
  }\
   \
  _eltType get(IloTuple index) const{\
    return super::get(index);\
  }
...
protected:
  Elt& at(IloInt index) const {
    IloOplAssert (_impl, "Empty Handle in IloMap::get/set/at");
  ...
  IloSubMapI elt = CreateData(env, currentDim+1, indexer);
  a[i]= elt;
      }
...
public:
  ...
  IloMap(IloEnv env, IloDiscreteDataCollection indexer) {
    _impl = new (env) IloMapI(env.getImpl(), 1,
            indexer.getImpl(), (IloInt)sizeof(Elt)) ;
  ...
  IloMap(IloEnv env, IloMapIndexer indexer) {
    _impl = CreateData(env.getImpl(), 0, indexer);
  }

  void end() {
    IloOplAssert(_impl, "Empty Handle in IloMap::end");
    delete _impl;
    _impl = 0;
  }

  IloEnv getEnv() const {
    IloOplAssert(_impl, "Empty Handle in IloMap::getEnv");
    return _impl->getEnv();
  }

  
  IloInt getNbDim() const {
    IloOplAssert(_impl, "Empty Handle in IloMap::getNbDim");
    return _impl->getNbDim();
  }
  
  IloInt getSize() const {
    IloOplAssert(_impl, "Empty Handle in IloMap::getSize");
    return _impl->getSize();
  }
  
  IloInt getTotalSize() const {
    IloOplAssert(_impl, "Empty Handle in IloMap::getTotalSize");
  ...
  IloDiscreteDataCollection getIndexer() const {
    IloOplAssert(_impl, "Empty Handle in IloMap::getIndexer");
  ...
  IloSubMapI getAtAbsoluteIndex(IloInt i) const {
    IloOplAssert (_impl, "Empty Handle in IloMap::getAtAbsoluteIndex");
  ...
  IloMap<Elt> operator[](IloInt i) const {
    IloOplAssert (_impl, "Empty Handle in IloMap::operator[]");
  ...
  IloMap<Elt> getSub(IloNum n) const {
    IloOplAssert (_impl, "Empty Handle in IloMap::set");
  ...
  Elt& eltAtAbsoluteIndex(IloInt i) const {
    IloOplAssert(_impl, "Empty Handle in IloMap::eltAtAbsoluteIndex");
  ...
  void setAt(IloMapIndexArray indices, Elt value) {
    IloOplAssert (_impl, "Empty Handle in IloMap::setAt");
  ...
  Elt getAt(IloMapIndexArray indices) const {
    IloOplAssert (_impl, "Empty Handle in IloMap::getAt");
    IloOplAssert(indices.isConstant(), "IloMap::getAt does not accept non constant indices.");
  ...
  IloMapI* copy() const {
    IloOplAssert (_impl, "Empty Handle in IloMap::copy");
  ...
  public:
    ...
    void operator delete(void*, const IloGenAlloc&) {}
    void operator delete(void*, const IloGenAlloc*) {}
#endif

    void operator delete(void *p, size_t sz) {
    Iterator* doomed = (Iterator*)p;
    doomed->getHeap()->free(p, sz);
  }
  };

  Iterator* iterator(IloGenAlloc* heap) const {
    IloOplAssert(getImpl() != 0, "IloMap: Using empty handle");
  return new (heap) Iterator(heap, _impl);
...
public:
  ...
  IloTupleSchema getSchema() const {
    IloOplAssert(getImpl(), "IloTupleSetMap::getSchema using empty handle");
    return IloTupleSchema(getImpl()->getAuxData());
  }
  #ifdef CPPREF_GENERATION
...
public:
  ...
  void visitSubExtractables(IloExtractableVisitor* v, IloExtractableI* e){
    IloOplAssert(getImpl(), "Empty Handle in IloExtractableMap::visitSubExtractables");
...
public:
  ...
  virtual IloDiscreteDataCollectionI* getIndexer(IloInt i) const ILO_OVERRIDE;
  virtual const char* getName() const ILO_OVERRIDE { return _name; }
  virtual void setName(const char* name) ILO_OVERRIDE;
  virtual IloInt getNbDim() const ILO_OVERRIDE;
  virtual IloInt getTotalSize() ILO_OVERRIDE;
...
public:
  ...
  virtual IloBool isDExpr() ILO_OVERRIDE {
    return IloTrue;
  }
  IloBool hasDExprMap() const { return _dexprmap != 0; }
  IloIntExprMapLightI* getDExprMap() const { return _dexprmap; }
  void setDExprMap(IloIntExprMapLightI* m) { _dexprmap = m; }
};
...
public:
  ...
  IloIntExprArg get(IloTuple index) const {
    IloOplAssert(_impl, "Using empty IloIntDExprMap handle");
...
public:
  ...
  IloIntFixedArray getFixedArray() { return  _index; }
...
public:
  ...
  IloBool hasDExprMap() const { return _dexprmap != 0; }
  IloNumExprMapLightI* getDExprMap() const { return _dexprmap; }
  void setDExprMap(IloNumExprMapLightI* m) { _dexprmap = m; }
};
...
public:
  ...
  IloNumExprArg get(IloTuple index) const {
    IloOplAssert(_impl, "Using empty IloNumDExprMap handle");
    IloMapIndexArray a = _impl->getSharedUnaryMapIndexArray();
  a[0] = index;
