...
public:
  IloTupleDefinitionI(IloEnv env, IloTupleSchemaI const *schema,
                      const char* name = 0);
  virtual ~IloTupleDefinitionI();
  virtual IloBool isTuple() const ILO_OVERRIDE { return IloTrue; }
...
protected :
  virtual void change() const ILO_OVERRIDE = 0;
...
class IloTupleCollectionChangeNotifierI : public IloDataChangeNotifierI {
  ...
  IloTupleSetI *const _tupleSet;
protected:
  virtual void change() const ILO_OVERRIDE;
...
public:
  virtual void change() const ILO_OVERRIDE;
  virtual void changeTuple(IloInt, IloTupleCellArray) const ILO_OVERRIDE;
  virtual void addTuple(IloInt) const ILO_OVERRIDE;
  virtual void removeTuple(IloInt) const ILO_OVERRIDE;
  virtual void removeCollection(IloTupleCollectionI* coll) const ILO_OVERRIDE;
private:
...
private:
  ...
  IloTupleCellArray initNormalizedCommitCells(IloTupleCellArray line);
  IloTupleCellArray initNormalizedFindCells(IloTupleCellArray line);
  class NormalizedCells : public IloTupleCellArray {
    NormalizedCells(NormalizedCells const &); 
    NormalizedCells &operator=(NormalizedCells const &); 
    IloTupleCollectionI *const _coll;
    IloTupleCellArray _line;
  protected:
    NormalizedCells(IloTupleCellArray (IloTupleCollectionI::*f)(IloTupleCellArray),
                    IloTupleCollectionI *coll, IloTupleCellArray(line))
      : IloTupleCellArray((coll->*f)(line)), _coll(coll), _line(line) {}
  public:
    ~NormalizedCells() { _coll->releaseNormalizedCells(_line); }
  };
protected:
  struct NormalizedCommitCells : public NormalizedCells {
    NormalizedCommitCells(IloTupleCollectionI *coll, IloTupleCellArray line)
      : NormalizedCells(&IloTupleCollectionI::initNormalizedCommitCells,
                        coll, line)
    {}
  };
  struct NormalizedFindCells : public NormalizedCells  {
    NormalizedFindCells(IloTupleCollectionI *coll, IloTupleCellArray line)
      : NormalizedCells(&IloTupleCollectionI::initNormalizedFindCells,
                        coll, line)
    {}
  };
...
public:
    IloTupleBufferI* getOrMakeSharedTupleBuffer(){
      IloTupleBufferI* b = getSchema()->getOrMakeSharedTupleBuffer();
  ...
  void releaseNormalizedCells(IloTupleCellArray line);
  void checkReferences() const;

protected:
  void setListeners(IloTupleSetI*);
  void init(IloEnv env, IloTupleSchemaI const *schema, IloInt n = 0);
...
public:
  ...
  IloBool hasReference() const ILO_OVERRIDE { return _hasReference;}
  ...
  IloIntArray getCollectionColumnIndexes() const{ return getSchema()->getCollectionColumnIndexes();}
  ...
  IloTupleCollectionI(IloEnv env, IloTupleSchemaI const *schema, IloInt numberOfLine, IloTupleCellArray defaultValue);
  IloTupleCollectionI(IloEnv env, IloTupleSchemaI const *schema, IloInt numberOfLine); 
...
public:
  virtual void setLockable(IloBool flag) ILO_OVERRIDE;
...
public:
  ...
  inline IloTupleCellArray getOrMakeEmptySharedTupleCells() {
    if (!_sharedTupleCells.getImpl()){
      _sharedTupleCells = IloTupleCellArray(getEnv()->getMemoryManager(), getSchema()->getTotalColumnNumber());
      return _sharedTupleCells;
    }
    IloInt size = getSchema()->getTotalColumnNumber();
  ...
  inline IloTupleCellArray getOrMakeEmptySharedKeyCells() {
    if (!_sharedKeyCells.getImpl()){
      _sharedKeyCells = IloTupleCellArray(getEnv()->getMemoryManager(), getSchema()->getOrMakeTotalKeyIndexes().getSize());
      return _sharedKeyCells;
    }
    IloInt size = getSchema()->getOrMakeTotalKeyIndexes().getSize();
...
public:
  ...
  IloTupleCollectionI* getTupleColumn(IloIntArray const &path) const;
  IloTupleCollectionI* getTupleColumn(IloInt size, IloInt const* path) const;

  IloInt getColumnIndex(const char* name) const;
  IloInt getColumnIndex(const IloSymbolI* s1) const;
  virtual IloBool supportDuplicates() const ILO_OVERRIDE { return IloTrue; }
  IloTupleSchemaI const *getSchema() const { return _schema;}
  void setSchema(IloTupleSchemaI const *schema) { _schema = schema; }
  ...
  virtual IloBool isTupleCollection() const ILO_OVERRIDE;
  inline IloInt getSize() const ILO_OVERRIDE {
    return _size;
  }
  ...
  IloDataIterator* iterator(IloGenAlloc* heap) const ILO_OVERRIDE;
...
public:
  ...
  ILORTTIDECL
    virtual IloInt commit(IloTupleCellArray array) ILO_OVERRIDE {
...
public:
  ...
  virtual IloTupleCollectionChangeNotifierI* getChangeNotifier() ILO_OVERRIDE;
...
public:
  ...
  virtual void empty() ILO_OVERRIDE { clear(); }
  virtual void clear() ILO_OVERRIDE;
  ...
  virtual IloBool isTupleSet() const ILO_OVERRIDE;
  IloTupleSetI(IloEnv env, IloTupleSchemaI const *schema);
  IloTupleSetI(IloEnv env, IloTupleSchemaI const *schema, IloTupleCellArray defaultValue);
  ...
  virtual IloInt commit(IloTupleBuffer line) ILO_OVERRIDE;
  ...
  virtual IloInt commit(IloTuplePatternI*) ILO_OVERRIDE;
  ...
  virtual void remove(IloTupleCellArray buffer) ILO_OVERRIDE;
  virtual IloBool areEquivalents(IloInt index1, IloInt index2) const ILO_OVERRIDE;
  IloDiscreteDataCollectionI* smartCopy() const;

public:
  virtual void sort(IloBool  = IloTrue) ILO_OVERRIDE {}
  virtual IloBool isKeyEnabled() const ILO_OVERRIDE;

private:
  virtual void makeInt2Cells() ILO_OVERRIDE;
  virtual IloTupleCollectionI* cleanMe() ILO_OVERRIDE;
};
