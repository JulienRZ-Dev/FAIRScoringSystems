...
public:
  ...
  IloNumCollectedMapWrongSizeException (const IloNumCollectedMapI*,
                                        ...
                                        IloInt size,
                                        const IloExtractableI*);
  IloInt getExpectedSize() const { return _expectedSize; }
...
class IloInvalidSequenceConstraintException : public IloException {
  const IloExtractableI* _context;
public:
  IloInvalidSequenceConstraintException(const IloExtractableI* context);
  void print(ILOSTD(ostream)& out) const ILO_OVERRIDE;
  const IloExtractableI* getContext() const { return (_context); }
};
