...
class IloExcelInstanceError : public IloException {
  ...
  LONG _code;
public:
  IloExcelInstanceError(LONG code = 0);
  IloExcelInstanceError &operator=(IloExcelInstanceError const &);
  virtual ~IloExcelInstanceError();
  const char* getMessage() const ILO_OVERRIDE { return _buffer; }
...
public:
  ...
  const char* getStringCell(long vertOffset, long horizOffset);
...
public:
	...
	IloExcelException( const IloExcelException& e )
		: IloException(e), _row( e._row ), _column( e._column ) {}
