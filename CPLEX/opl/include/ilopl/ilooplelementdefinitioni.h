...
public:
    ...
    void displayType(std::ostream& outs) const;
	...
	virtual IloBool isDiscreteDataCollection() const {
		return IloFalse;
	}
	virtual IloBool isMap() const {
		return IloFalse;
	}
...
public:
	...
	virtual IloBool isDecisionVariable() const ILO_OVERRIDE {
		return _isDvar;
	}
};
...
public:
	...
	virtual IloBool isDecisionExpression() const ILO_OVERRIDE {
		return getItem().isDecisionExpression();
	}
	virtual IloBool isMap() const ILO_OVERRIDE {
		if (isDecisionVariable()) return IloFalse;
		if (isDecisionExpression()) return IloFalse;
		return IloTrue;
	}

    virtual const IloOplElementDefinitionI& getLeaf() const ILO_OVERRIDE {
...
public:
	IloOplDecisionExprDefinitionI(IloEnvI* env, const char* name, Type defType, const IloOplLocationI& location, IloBool isPost)
        :IloOplElementDefinitionI(env,name,defType,location,IloFalse,isPost), _usedInObjective(IloFalse), _simpleGoal(0), _simpleGoalCoef(0), _simpleGoalIsMinimize(IloFalse), _used(0), _isNonLinear(IloFalse) {
			setInternalData(IloTrue);
	}
