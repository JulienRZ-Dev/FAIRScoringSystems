...
public:
  ...
  IloIntIndex getIndex() const {
    IloOplAssert(getImpl(), "Using empty IloIntGenerator handle.");
...
public:
  ...
  IloNumIndex getIndex() const {
    IloOplAssert(getImpl(), "Using empty IloNumGenerator handle.");
    return (getImpl()->getIndex());
  }
};
...
public:
  ...
  IloSymbolIndex getIndex() const {
    IloOplAssert(getImpl(), "Using empty IloSymbolGenerator handle.");
...
public:
  ...
  IloTupleIndex getIndex() const {
    IloOplAssert(getImpl(), "Using empty IloTupleGenerator handle.");
...
ILO_EXPORTEDFUNCTION(IloNumExprArg) IloSlack(const IloIntExprArg ct);
ILO_EXPORTEDFUNCTION(IloNumExprArg) IloDual(const IloIntExprArg ct);
ILO_EXPORTEDFUNCTION(IloNumExprArg) IloReducedCost(const IloNumExprArg var);
