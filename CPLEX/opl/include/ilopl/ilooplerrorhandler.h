...
public:
    ...
    IloOplErrorHandler(IloEnv env, std::ostream& outs):_impl(0) {
	...
	void warning( const char* id, char* param1, char* param2, char* param3 );
...
protected:
    ...
    virtual IloBool handleAssertFail( const char* label, const IloStringArray& names, const IloMapIndexArray& values, const IloOplLocationI& location) ILO_OVERRIDE {
...
public:
    ...
    virtual IloBool ok() const ILO_OVERRIDE {
...
inline
ILOOPL_EXPORTEDFUNCTION(std::ostream&) operator<<( std::ostream&  os, const IloOplLocation&  location) {
