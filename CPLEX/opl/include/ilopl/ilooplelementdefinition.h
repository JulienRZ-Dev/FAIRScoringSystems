...
public:
	...
	IloInt getElementDefinitionType_asInt() const{
		return (IloInt)getElementDefinitionType();
	}
...
public:
	...
	IloOplSetDefinition(const IloOplSetDefinitionI* impl = 0):IloOplElementDefinition(impl) {
	}
...
public:
	...
	IloOplTupleSchemaDefinition(const IloOplTupleSchemaDefinitionI* impl = 0):IloOplElementDefinition(impl) {
	}
...
public:
	...
	IloOplTupleDefinition(const IloOplTupleDefinitionI* impl = 0):IloOplElementDefinition(impl) {
	}
...
protected:
    ...
    virtual void notifyElementDefinition(const IloOplElementDefinitionI& def) ILO_OVERRIDE {
        notifyElementDefinition(IloOplElementDefinition(&def));
    }

    virtual void notifyPushConstraint(const char* name, const IloOplLocationI& location) ILO_OVERRIDE {
        notifyPushConstraint(name,IloOplLocation(&location));
    }

    virtual void notifyExecute(const char* name, const IloOplLocationI& location) ILO_OVERRIDE {
        notifyExecute(name,IloOplLocation(&location));
    }

    virtual void notifyAssert(const char* name, const IloOplLocationI& location) ILO_OVERRIDE {
        notifyAssert(name,IloOplLocation(&location));
    }

	virtual void notifyObjective(IloBool simple, IloBool minimize, const IloOplLocationI& location) ILO_OVERRIDE {
        notifyObjective(simple, minimize, IloOplLocation(&location));
    }
    
	virtual IloBool isVerbose() const ILO_OVERRIDE {
		return _verbose;
	}
...
public:	
	...
	virtual void notifyClosePreProcessing() ILO_OVERRIDE =0;

    
	virtual void notifyPushConstraint(const char* name, IloOplLocation location) =0;
    
	virtual void notifyPopConstraint() ILO_OVERRIDE =0;
